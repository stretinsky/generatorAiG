...
namespace GenaratorAiG.Tasks.SLAE
{
    internal class Task_80
    {
        ...
        int[,] a,intermediate,reverse = new int[3,3];
        int determinant = 0;
        ...
        public Task_80()
        {
            ...
            {
                CreateMatrix();
                determinant = a[0, 0] * a[1, 1] * a[2, 2] + a[0, 1] * a[1, 2] * a[2, 1] + a[0, 2] * a[1, 0] * a[2, 1] -
        ...
        {
            return $"\\frac{{1}}{{{determinant}}} \\matrix{{{a[0, 0]} & {a[0, 1]} & {a[0, 2]} " +
                $"\\\\ {a[1, 0]} & {a[1, 1]} & {a[1, 2]} " +
                $"\\\\ {a[2, 0]} & {a[2, 1]} & {a[2, 2]}}}"; ;
        }
        public string GetAnswer()
        {
            return $"\\frac{{1}}{{{determinant}}} \\matrix{{{reverse[0,0]} & {reverse[0,1]} & {reverse[0,2]} " +
